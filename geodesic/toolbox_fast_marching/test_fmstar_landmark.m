...
for s=1:nbr_start_points
    ...
    [D,S] = perform_fast_marching_2d(W, start_points{s}, options);
...
for i = 1:nbr_paths
    ...
    for k = prev_nbr_landmarks+1:nbr_landmarks
        DL(:,:,k) = perform_fast_marching_2d(W, landmark(:,k));
    end
    % perform the FM*
    for s=1:nbr_start_points
...
options.heuristic = perform_fast_marching_2d(W, end_points{1});
