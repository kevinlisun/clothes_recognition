...
if isfield(options, 'v2v')temp
    v2vElem = options.v2v{x};
else
    v2vElem = vrc.compute_vertex_ring_element(x);
end
% initialize the paths
[w,f] = vertex_stepping(face, x, e2f, v2vElem, D);
vlist = [x;w]; 
plist = [1];
Dprev = D(x);
while true;
    ...
    elseif etat<0 && t>=0 && t<=1 && f2>0
        ...
        v2vElem = vrc.compute_vertex_ring_element(z);
        [w,f] = vertex_stepping( face, z, e2f, v2vElem, D);
        vlist(:,end+1) = [z w];  
        plist(end+1) = 1;   
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [w,f] = vertex_stepping(face, v, e2f, v2vElem, D)

% adjacent vertex with minimum distance
[tmp,I] = min( D(v2vElem) ); w = v2vElem(I);
f1 = e2f(v,w);
f2 = e2f(w,v);
if f1<0
    f = f2; return;
