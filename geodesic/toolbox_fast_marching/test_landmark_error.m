...
[D,S] = perform_fast_marching_2d(W, end_points);
clf;
hold on;
m = sort(D(:)); m = m(end-10);
imagesc(min(D,m)');
...
for s=1:nbr_base_points
    fprintf('.');
    [DL_base(:,:,s),Z] = perform_fast_marching_2d(W, base_points(:,s));
end
fprintf('\n');
    ...
    for nbr_landmarks=nbr_landmark_list{k1}
        ...
        while size(landmark,2)<nbr_landmarks
            ...
            switch lower(landmark_init)
                ...
                case 'farthest'
                    landmark = farthest_point_sampling( W, landmark, 1 );
                case 'trialerror'
                    landmark = farthest_landmark_sampling( W, landmark, DL_landmark, base_points, DL_base, 1 );
                case 'farthestboundary'
                    landmark = farthest_point_sampling_boundary( W, landmark, 1 );
                case 'farthestunif'
                    landmark = farthest_point_sampling_boundary( W, landmark, 1, 'unif' );
            end
            
            % update distance map
            [DL1,S] = perform_fast_marching_2d(W, landmark(:,end));
